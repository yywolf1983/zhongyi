# 全量数据核查与修复报告（所有 JSON）
> ⚠️ 本报告记录的是 **JSON 时代**的全量核查过程；数据现已迁移至 SQLite（`db/zhongyi.db` / `public/zhongyi.db`）。文中记录数（如 formulas 360、medicines 577、acupoints 417、effects 753、mapping 180 等）均为当时快照，当前以 SQLite 为准（证型 131 / 方剂 490 / 中药 628 / 穴位 418 / 针方 371 / 经络 15 / 治法 30 / 功效 917 / 中西对照 179，合计 3179）。流程与方法仍可参考，但结论不代表现有数据规模。

> 完成时间：2026-07-17 ｜ 覆盖 9 个数据文件（JSON 时代）
...
## 七、本轮（内容正确性专项审计 · 方剂/针方/药材）

> 触发：用户指出"桂枝汤的方剂是错误的"，要求核查全部数据中的内容错误。

### 7.1 根因：药材/穴位 ID 错位（系统性数据损坏）
- 药材表由约 360 味扩充至 574+ 味、穴位表扩充后，**ID 整体后移，但方剂/针方仍引用旧 ID**，导致 `medicine_id`/`acupoint_id` 指向了错误的药/穴。
- 影响规模（修复前）：**方剂 236 处** + **针方 36 处** ID 与名称不符。
- 典型错误：桂枝汤"大枣"指向 `艾叶`（medicine_056）；麻黄汤"甘草"指向 `黄芪`；颊车指向"上廉"等。
- 修复：以各条 `name` 为权威，对照当前药材/穴位表**重新解析 `medicine_id`/`acupoint_id`**。
  - 方剂修正 **220 处**；针方修正 **19 处**。
  - 针方中 17 处为"阿是穴/颈夹脊/夹脊穴/局部围刺/百劳"等无固定穴位的奇穴/阿是穴，本无规范 ID，予以保留（仅名称标注）。
  - 药材变体名（麻子仁→火麻仁、龟板→龟甲、川牛膝→牛膝、生龙骨→龙骨…）保留原名称、ID 指向正药，均为正确关联。

### 7.2 桂枝汤（formula_002）专项修正
- 原数据**缺炙甘草**（经典桂枝汤 = 桂枝、白芍、生姜、大枣、炙甘草），已补 `炙甘草 6g 使`。
- 原"大枣"的 `medicine_id` 错指 `艾叶`，已随 7.1 一并纠正。
- 现 5 味药材 ID 全部正确（桂枝/白芍/生姜/大枣/炙甘草）。

### 7.3 名方组成截断/错药修正（160 首）
- 数据生成时 328 首方剂被"截断"且部分含**错药**（如小青龙汤混入"杏仁"、缺芍药/细辛/干姜/半夏/五味子）。
- 以《伤寒论》《金匮要略》《温病条辨》《太平惠民和剂局方》及公认名方为底，建立**权威组成参考**，对 **160 首**最常用经典名方**整体替换**为完整、正确的组成（含标准用量与君臣佐使）。
- 例如：银翘散补至 10 味、小柴胡汤补至 7 味、归脾汤补至 12 味、血府逐瘀汤补至 11 味、六味地黄丸补至 6 味、龙胆泻肝汤补至 10 味、补中益气汤补至 8 味等，并已剔除小青龙汤的错药"杏仁"。
- 修正中发现的 2 处误映射（瓜蒌根→应天花粉、沙苑蒺藜→应沙苑子）已单独更正。

### 7.4 药材（medicines）内容核查
- 结构校验：归经字段原混用"心/肺/肝胆经"等简称，统一规范为"X经"（**50 首归一、43 首拆分合并式**），最终 **0 条**无效归经。
- 四气五味抽查（桂枝、麻黄、甘草、白芍、黄芩、黄连、附子、石膏、柴胡、半夏等）均正确。
- 补入 3 味经典方实际用药但原表缺失的药材：**猪脊髓、黄酒、糯米**（medicine_575/576/577），并回填至对应方剂，消除空 ID。

### 7.5 最终终态（复检）
- **悬空引用 = 0**（9 文件全检）。
- **双向不对称 = 0**（全部对称）。
- 记录数：`formulas` 360 ｜ `needle` 371 ｜ `syndromes` 131 ｜ `effects` 753 ｜ `medicines` 577 ｜ `acupoints` 417 ｜ `meridians` 15 ｜ `treatments` 30 ｜ `mapping` 180。
- 方剂药材 ID 与名称一致（仅保留变体别名映射，均正确）。

### 7.6 仍需专家复核（非程序可判定）
- **192 首方剂**组成仍标记"截断"：属 160 首权威名方之外的长尾（含大量时方、专科方），其完整组成需中医师按原著逐首补全（本轮已确保现有药味名称正确、ID 正确，仅可能"不全"而非"有误"）。
